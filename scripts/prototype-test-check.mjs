...
runtime('apps/admin/app.js', '#/login', ({ app, visit, click, change, search, inputDraft, changeDraft, inputLogin }) => {
  ...
  if (!app.innerHTML.includes('license-grid') || !app.innerHTML.includes('占个人用户') || !app.innerHTML.includes('license-ring')) errors.push('工作台执照数据未按分类卡片排版');
  ...
  if (!app.innerHTML.includes('从用户选择') || !app.innerHTML.includes('手工录入') || !app.innerHTML.includes('选择用户') || !app.innerHTML.includes('data-action="volunteer-entry-mode"') || !app.innerHTML.includes('姓名 / 手机号') || !app.innerHTML.includes('device-picker') || !app.innerHTML.includes('常住地址') || !app.innerHTML.includes('admin-cascader') || !app.innerHTML.includes('data-action="toggle-region-cascader"') || !app.innerHTML.includes('data-action="pick-region-cascader"') || !app.innerHTML.includes('data-draft-field="addressDetail"') || app.innerHTML.includes('admin-region-panel') || app.innerHTML.includes('region-cascade-select') || app.innerHTML.includes('data-draft-field="street"')) errors.push('添加志愿者缺少从用户选择/手工录入双模式、用户搜索或常住地址级联选择器');
  click({ action: 'toggle-region-cascader' });
  if (!app.innerHTML.includes('admin-cascader-panel') || !app.innerHTML.includes('data-level="province"') || app.innerHTML.includes('admin-region-panel')) errors.push('常住地址级联选择器未展开省市区面板');
  click({ action: 'pick-region-cascader', level: 'province', value: '浙江省' });
  if (!/admin-cascader-panel[^"]*\bhas-city\b/u.test(app.innerHTML) || !app.innerHTML.includes('data-level="city"')) errors.push('常住地址级联选择器选中省后未展开市列');
  click({ action: 'pick-region-cascader', level: 'city', value: '宁波市' });
  if (!/admin-cascader-panel[^"]*\bhas-district\b/u.test(app.innerHTML) || !app.innerHTML.includes('data-level="district"')) errors.push('常住地址级联选择器选中市后未展开区列');
  click({ action: 'pick-region-cascader', level: 'district', value: '鄞州区' });
  if (!app.innerHTML.includes('浙江省 / 宁波市 / 鄞州区') || app.innerHTML.includes('admin-cascader is-open')) errors.push('常住地址级联选择器选中区后未回填');
  click({ action: 'volunteer-pick-user', id: 'USR-001' });
  if (!app.innerHTML.includes('已选用户') || !app.innerHTML.includes('鄞州区') || !app.innerHTML.includes('示范路 128 号') || app.innerHTML.includes('data-draft-field="street"')) errors.push('从个人用户选择志愿者后未带入常住地址');
