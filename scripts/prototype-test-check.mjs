...
runtime('apps/zheliban/app.js', '#/login', ({ app, visit, click, change, upload, uploadFeedback, input, inputGuideSearch, inputFaqSearch, inputProfile, inputCompany, changeCompany, inputMember, inputDrone, changeDrone, setValidity }) => {
  ...
  if (!['常住地址', '紧急联系人', '紧急联系电话', '浙江省 / 宁波市 / 鄞州区', '请填写详细地址', '下应街道', 'data-action="open-region-picker"', 'data-supplement-field="addressDetail"', 'region-trigger'].every((field) => app.innerHTML.includes(field)) || app.innerHTML.includes('所属地') || app.innerHTML.includes('常驻地址') || app.innerHTML.includes('请选择街道') || app.innerHTML.includes('data-supplement-field="province"') || app.innerHTML.includes('data-supplement-field="regionPath"') || app.innerHTML.includes('data-supplement-field="street"') || app.innerHTML.includes('常用飞行区域') || app.innerHTML.includes('usualArea') || app.innerHTML.includes('请选择鄞州区街道')) errors.push('个人补充信息未改为常住地址省市区联动选择与手填');
  click({ action: 'open-region-picker' });
  if (!['region-picker-layer', 'data-action="pick-region"', 'data-level="province"', 'data-level="city"', 'data-level="district"', 'data-action="confirm-region-picker"', '北京市', '河北省', '鄞州区'].every((item) => app.innerHTML.includes(item))) errors.push('个人补充信息省市区三列联动选择器缺失');
  click({ action: 'pick-region', level: 'province', value: '北京市' });
  if (!app.innerHTML.includes('data-value="东城区"') || !app.innerHTML.includes('data-level="city"')) errors.push('个人补充信息切换省后未联动市/区');
  ...
  click({ action: 'confirm-region-picker' });
  if (!app.innerHTML.includes('浙江省 / 宁波市 / 鄞州区')) errors.push('个人补充信息恢复鄞州区选择失败');
...
runtime('apps/admin/app.js', '#/login', ({ app, visit, click, change, search, inputDraft, changeDraft, inputLogin }) => {
  ...
  if (!app.innerHTML.includes('license-grid') || !app.innerHTML.includes('占应持有') || !app.innerHTML.includes('license-ring')) errors.push('工作台执照数据未按独立双卡排版');
  if ((app.innerHTML.indexOf('dashboard-license') < 0) || (app.innerHTML.indexOf('dashboard-license') > app.innerHTML.indexOf('dashboard-flight'))) errors.push('工作台执照数据应独立于飞行报备统计区之前展示');
  ...
  click({ action: 'close-modal' });
  visit('#/districts');
  if (!app.innerHTML.includes('市区配置') || !app.innerHTML.includes('新增市区') || !app.innerHTML.includes('鄞州区') || !app.innerHTML.includes('海曙区') || !['编号', '名称', '排序', '状态', '更新时间'].every((field) => app.innerHTML.includes(field))) errors.push('市区配置列表字段或预置数据不正确');
  click({ action: 'modal', modal: 'edit-config', key: 'districts', item: 'new' });
  if (!app.innerHTML.includes('新增市区') || !app.innerHTML.includes('data-draft-field="name"') || !app.innerHTML.includes('submit-config')) errors.push('市区配置新增弹窗不正确');
  click({ action: 'close-modal' });
  ...
  click({ action: 'close-modal' });
  const routes = {users:'用户管理',companies:'企业管理',blacklist:'黑名单','sys-users':'用户管理',roles:'角色管理',menus:'菜单管理',dicts:'字典管理',certificates:'UOM 登记证','drone-blacklist':'无人机黑名单',verification:'设备核查',streets:'街道配置',districts:'市区配置','flight-activity-types':'飞行活动类型配置',flights:'飞行计划',activities:'活动管理',laws:'低空安全普法',news:'新闻公告',guides:'操作手册',faq:'常见问题',volunteers:'志愿者名册',messages:'消息管理',feedback:'意见反馈'};
  Object.entries(routes).forEach(([name, heading]) => {
    visit(`#/${name}`);
