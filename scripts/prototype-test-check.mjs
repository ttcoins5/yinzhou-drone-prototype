...
if (!/\.phone-shell\.viewport-mobile \.modal\{[^}]*scrollbar-width:none/u.test(zhelibanCss) || !zhelibanCss.includes('.phone-shell.viewport-mobile .modal::-webkit-scrollbar{display:none}')) errors.push('移动端弹窗未隐藏系统滚动条视觉（应写入设计规范并在 CSS 中隐藏）');
...
runtime('apps/zheliban/app.js', '#/login', ({ app, visit, click, change, search, upload, uploadFeedback, input, inputGuideSearch, inputFaqSearch, inputProfile, inputCompany, changeCompany, inputMember, inputDrone, changeDrone, setValidity }) => {
  ...
  if (!app.innerHTML.includes('hero-banner-carousel') || !app.innerHTML.includes('安全培训开始报名') || !app.innerHTML.includes('起飞前请完成安全提示核对') || !app.innerHTML.includes('八月宣传月活动安排已发布')) errors.push('浙里办首页未轮播生效中的 Banner 推送文案');
  if (!app.innerHTML.includes('hero-banner-slide is-activity') || !app.innerHTML.includes('hero-banner-slide is-law') || !app.innerHTML.includes('hero-banner-slide is-news')) errors.push('首页 Banner 未按活动/普法/公告区分展示');
  if (app.innerHTML.includes('社区宣传活动即将开放') || app.innerHTML.includes('夏季飞行安全提示（已停用）')) errors.push('未生效或已停用的 Banner 不应出现在首页轮播');
  click({ action: 'open-banner', id: 'BAN-01' });
  if (!app.innerHTML.includes('2026 年鄞州区无人机飞行安全培训') || !app.innerHTML.includes('activity-detail')) errors.push('点击活动 Banner 未进入对应活动详情');
  click({ action: 'back', fallback: 'home' });
  click({ action: 'open-banner', id: 'BAN-02' });
  if (!app.innerHTML.includes('无人驾驶航空器飞行安全提示')) errors.push('点击普法 Banner 未进入对应详情');
  click({ action: 'back', fallback: 'home' });
  ...
  if (!app.innerHTML.includes('场地巡查') || app.innerHTML.includes('鄞州区河道巡检与隐患复核')) errors.push('个人飞行计划已执行筛选失败');
  ...
  if (app.innerHTML.includes('data-action="open-drone-create"') || app.innerHTML.includes('新增设备') || app.innerHTML.includes('filter-bar--with-action')) errors.push('无人机列表仍保留新增设备入口');
  ...
  visit('#/profile');
  if (!app.innerHTML.includes('my-account-hero') || !app.innerHTML.includes('account-mark personal') || !app.innerHTML.includes('个人账户（') || !app.innerHTML.includes('data-go="profile-details"') || !['我的执照','我的无人机','我的飞行申报','我报名的活动','意见反馈'].every((field) => app.innerHTML.includes(field)) || app.innerHTML.includes('>个人资料</b>') || ['身份证号','手机号码','飞行执照图片'].some((field) => app.innerHTML.includes(field))) errors.push('个人“我的”账户首页或服务入口未按要求重构');
  ...
  if (!app.innerHTML.includes('个人基本信息') || app.innerHTML.includes('class="page-title"') || !['姓名','身份证号','手机号码','所属公司','常住地址','紧急联系人','紧急联系电话'].every((field) => app.innerHTML.includes(field)) || app.innerHTML.includes('<span>地址</span>') || app.innerHTML.includes('常用飞行区域') || app.innerHTML.includes('所属地')) errors.push('个人资料二级页字段缺失、仍带大标题栏或不含地址');
  ...
  if (!['常住地址', '紧急联系人', '紧急联系电话', '浙江省 / 宁波市 / 鄞州区', '请填写详细地址', 'data-action="open-region-picker"', 'data-supplement-field="addressDetail"', 'region-trigger'].every((field) => app.innerHTML.includes(field)) || app.innerHTML.includes('所属地') || app.innerHTML.includes('常驻地址') || app.innerHTML.includes('请选择街道') || app.innerHTML.includes('data-supplement-field="province"') || app.innerHTML.includes('data-supplement-field="regionPath"') || app.innerHTML.includes('data-supplement-field="street"') || app.innerHTML.includes('常用飞行区域') || app.innerHTML.includes('usualArea') || app.innerHTML.includes('请选择鄞州区街道') || app.innerHTML.includes('has-street') || app.innerHTML.includes('data-level="street"')) errors.push('个人补充信息未改为常住地址省市区联动选择与手填详细地址');
  click({ action: 'open-region-picker' });
  if (!['region-picker-layer', 'data-action="pick-region"', 'data-level="province"', 'data-level="city"', 'data-level="district"', 'data-action="confirm-region-picker"', '北京市', '河北省', '鄞州区'].every((item) => app.innerHTML.includes(item)) || app.innerHTML.includes('has-street') || app.innerHTML.includes('data-level="street"') || app.innerHTML.includes('钟公庙街道')) errors.push('个人补充信息省市区联动选择器缺失或仍含街道列');
  click({ action: 'pick-region', level: 'province', value: '北京市' });
  if (!app.innerHTML.includes('data-value="东城区"') || !app.innerHTML.includes('data-level="city"') || app.innerHTML.includes('has-street')) errors.push('个人补充信息切换省后未联动市/区，或仍展示街道列');
  click({ action: 'pick-region', level: 'district', value: '东城区' });
  click({ action: 'confirm-region-picker' });
  if (!app.innerHTML.includes('北京市 / 北京市 / 东城区') || app.innerHTML.includes('region-picker-layer')) errors.push('个人补充信息确认省市区后未回填');
  click({ action: 'open-region-picker' });
  click({ action: 'pick-region', level: 'province', value: '浙江省' });
  click({ action: 'pick-region', level: 'city', value: '宁波市' });
  click({ action: 'pick-region', level: 'district', value: '鄞州区' });
  click({ action: 'confirm-region-picker' });
  if (!app.innerHTML.includes('浙江省 / 宁波市 / 鄞州区') || app.innerHTML.includes('浙江省 / 宁波市 / 鄞州区 / 下应街道')) errors.push('个人补充信息恢复鄞州区选择失败');
  if (app.innerHTML.includes('字段口径待业务确认') || app.innerHTML.includes('选填项，仅用于区级')) errors.push('个人补充信息弹窗不应展示字段口径说明');
  click({ action: 'close-modal' });
  visit('#/profile-license');
  if (app.innerHTML.includes('class="page-title"') || !app.innerHTML.includes('我的执照') || !app.innerHTML.includes('民用无人驾驶航空器操控员执照') || !app.innerHTML.includes('无人机驾驶员合格证') || !app.innerHTML.includes('超视距驾驶员执照')) errors.push('我的执照二级页缺失分类或仍带大标题栏');
  ...
  if (app.innerHTML.includes('data-action="open-drone-create"') || app.innerHTML.includes('新增设备')) errors.push('企业无人机列表仍保留新增设备入口');
  ...
  click({ action: 'flight-exec-view', value: '已执行' });
  if (!app.innerHTML.includes('邱隘镇仓储屋顶巡检') || app.innerHTML.includes('农田灌溉设施巡检')) errors.push('企业飞行计划已执行筛选失败');
  click({ action: 'flight-exec-view', value: 'all' });
  change({ flightRange: 'start' }, '2026-08-06');
  change({ flightRange: 'end' }, '2026-08-06');
...
runtime('apps/admin/app.js', '#/login', ({ app, visit, click, change, search, inputDraft, changeDraft, inputLogin }) => {
  ...
  if (!['个人用户', '企业用户', '在册无人机', '飞行计划', '起飞地分布', '执照数据', '飞行架次统计', '飞行活动类型统计', 'dashboard-flight', 'dashboard-license'].every((item) => app.innerHTML.includes(item))) errors.push('后台工作台未按数据统计口径展示指标、执照或飞行报备统计');
  if (app.innerHTML.includes('近 7 日飞行计划') || app.innerHTML.includes('飞行区域分布') || app.innerHTML.includes('line-chart') || app.innerHTML.includes('line-chart-values')) errors.push('后台工作台仍保留近 7 日折线或旧飞行区域分布口径');
  if (app.innerHTML.includes('class="line-chart-value"')) errors.push('后台折线数值仍使用可被拉伸的 SVG text');
  if (!app.innerHTML.includes('民用无人驾驶航空器操控员执照') || !app.innerHTML.includes('无人机驾驶员合格证') || !app.innerHTML.includes('超视距驾驶员执照') || app.innerHTML.includes('已上传执照') || app.innerHTML.includes('未上传执照') || app.innerHTML.includes('占应持有')) errors.push('工作台执照数据应按执照分类统计已上传数量');
  if (!app.innerHTML.includes('license-grid') || !app.innerHTML.includes('占个人用户') || !app.innerHTML.includes('license-ring') || !app.innerHTML.includes('license-card')) errors.push('工作台执照数据未按统一分类卡片排版');
  if ((app.innerHTML.indexOf('dashboard-flight') < 0) || (app.innerHTML.indexOf('dashboard-license') < 0) || (app.innerHTML.indexOf('dashboard-flight') > app.innerHTML.indexOf('dashboard-license'))) errors.push('工作台飞行架次统计应展示在执照数据之前');
  if (!app.innerHTML.includes('报备架次') || !app.innerHTML.includes('已执行架次') || !app.innerHTML.includes('未执行架次') || !app.innerHTML.includes('sortie-grid')) errors.push('工作台飞行架次统计字段或横向排版缺失');
  if (app.innerHTML.includes('+12.4%') || app.innerHTML.includes('status success">+12')) errors.push('后台工作台仍展示旧折线环比涨幅标签');
  if (!app.innerHTML.includes('data-action="dashboard-pick"') || !/metric-card[^"]*is-interactive/u.test(app.innerHTML)) errors.push('后台工作台缺少指标卡或图表点击交互');
  if (!['近一周', '一月', '一年', '自选时间段', 'data-action="dashboard-area-range"', 'data-dashboard-board-toolbar', 'range-filter--board'].every((item) => app.innerHTML.includes(item))) errors.push('后台工作台飞行报备统计缺少共用时间段筛选');
  click({ action: 'dashboard-area-range', range: 'month' });
  if (!app.innerHTML.includes('飞行报备统计已切换为一月') && !app.innerHTML.includes('当前统计时间段：一月') && !app.innerHTML.includes('当前统计时间段：<b>一月</b>')) errors.push('后台工作台时间段筛选切换失败');
  if (!app.innerHTML.includes('报备架次') || !(app.innerHTML.includes('>11<') || app.innerHTML.includes('>11</') || /报备架次[\s\S]*?>11</.test(app.innerHTML))) errors.push('后台工作台切换一月后架次/起飞地统计未按飞行计划更新');
  if (!app.innerHTML.includes('一般飞行活动') || !app.innerHTML.includes('特殊飞行活动')) errors.push('后台工作台飞行活动类型统计未展示计划活动类型');
  ...
  click({ action: 'dashboard-pick', kind: 'sortie', id: '报备架次', value: '4', label: '报备架次' });
  if (!app.innerHTML.includes('报备架次：4') && !app.innerHTML.includes('sortie-card is-interactive is-active') && !app.innerHTML.includes('is-active')) errors.push('后台工作台架次统计点击反馈失败');
  click({ action: 'dashboard-pick', kind: 'area', id: '下应街道', value: '2', label: '下应街道' });
  if (!app.innerHTML.includes('下应街道：2 项（按起飞地）') && !app.innerHTML.includes('dist-row is-interactive is-active')) errors.push('后台工作台起飞地分布点击反馈失败');
  ...
  if (!app.innerHTML.includes('已加入无人机黑名单') || !app.innerHTML.includes('data-id="DR-001"') || !app.innerHTML.includes('>已禁用<')) errors.push('后台设备禁用 mock 交互失败或未置灰');
  visit('#/drone-blacklist');
  if (!app.innerHTML.includes('无人机黑名单') || !app.innerHTML.includes('云翼 M30') || !app.innerHTML.includes('测试禁用拉黑原因') || !app.innerHTML.includes('安巡 H20')) errors.push('无人机黑名单列表未收录禁用设备或预置数据');
  if (app.innerHTML.includes('>已取消<') || app.innerHTML.includes('id="state-filter"')) errors.push('无人机黑名单不应保留已取消状态筛选或已取消记录');
  const droneBlacklistTarget = [...app.innerHTML.matchAll(/data-action="request-change" data-key="drone-blacklist" data-id="([^"]+)"/g)].map((match) => match[1]).find((id) => id !== 'DBL-001');
  if (!droneBlacklistTarget) errors.push('无人机黑名单缺少刚禁用设备的取消拉黑入口');
  click({ action: 'request-change', key: 'drone-blacklist', id: droneBlacklistTarget || 'DBL-002' });
  if (!app.innerHTML.includes('确认取消拉黑') || !app.innerHTML.includes('云翼 M30')) errors.push('无人机黑名单取消拉黑确认弹窗缺失');
  click({ action: 'submit-unblacklist' });
  if (!app.innerHTML.includes('已取消拉黑') || !app.innerHTML.includes('恢复为正常')) errors.push('无人机取消拉黑反馈缺失');
  visit('#/drone-blacklist');
  if ((droneBlacklistTarget && app.innerHTML.includes(`data-id="${droneBlacklistTarget}"`)) || app.innerHTML.includes('测试禁用拉黑原因')) errors.push('取消拉黑后设备仍留在无人机黑名单列表');
  if (!app.innerHTML.includes('安巡 H20') || !app.innerHTML.includes('data-id="DBL-001"')) errors.push('取消拉黑误删了其他黑名单记录');
  visit('#/drones');
  if (!app.innerHTML.includes('data-id="DR-001"') || !app.innerHTML.includes('禁用设备')) errors.push('取消拉黑后无人机管理未恢复可禁用入口');
  if (!(app.innerHTML.includes('>正常<') || app.innerHTML.includes('status success">正常'))) errors.push('取消拉黑后无人机管理状态未恢复为正常');
  ...
  visit('#/detail/users/USR-002');
  if (!app.innerHTML.includes('我的执照') || !app.innerHTML.includes('民用无人驾驶航空器操控员执照') || !app.innerHTML.includes('无人机驾驶员合格证')) errors.push('后台用户详情未按分类展示已上传执照');
  visit('#/detail/users/USR-001');
  if (!['基本信息','补充信息','常住地址','紧急联系人','紧急联系电话','下应街道'].every((field) => app.innerHTML.includes(field)) || app.innerHTML.includes('所属地') || app.innerHTML.includes('常驻地址')) errors.push('后台用户详情未按用户端口径展示基本信息与补充信息');
  ...
  if (!['飞行活动类型','任务性质','操控模式','飞行模式','通信联络方式','最大飞行高度','起飞地','审批材料','计划状态','执行状态','执行时间','提交人','修改历史与执行记录'].every((field) => app.innerHTML.includes(field)) || app.innerHTML.includes('起降备降场地')) errors.push('后台飞行计划详情未按用户端口径展示字段');
  ...
  if (app.innerHTML.includes('计划名称') || !app.innerHTML.includes('内容标题') || !app.innerHTML.includes('发布单位') || !app.innerHTML.includes('阅读量')) errors.push('后台新闻列表字段未与详情内容对齐');
  visit('#/laws');
  if (!app.innerHTML.includes('生效起止') || !app.innerHTML.includes('2026-07-21 至 2027-07-20')) errors.push('后台法规列表未展示生效起止区间');
  if (app.innerHTML.includes('计划名称') || !app.innerHTML.includes('内容标题') || !app.innerHTML.includes('发布单位') || !app.innerHTML.includes('阅读量')) errors.push('后台法规列表字段未与详情内容对齐');
  ...
  {
    const contentNav = app.innerHTML.match(/data-group="活动与宣传科普"[\s\S]*?<\/div><\/div>/u)?.[0] || '';
    if (!['data-go="activities"', 'data-go="laws"', 'data-go="news"', 'data-go="banners"'].every((item) => contentNav.includes(item))) errors.push('活动、普法、公告与 Banner 推送未合并到侧栏「活动与宣传科普」');
    if (app.innerHTML.includes('data-group="宣传科普管理"') || app.innerHTML.includes('data-group="活动管理"')) errors.push('侧栏仍保留拆分的活动管理或宣传科普管理分组');
  }
  visit('#/banners');
  if (!app.innerHTML.includes('Banner 推送') || !app.innerHTML.includes('推送类型') || !app.innerHTML.includes('生效时段') || !app.innerHTML.includes('展示状态') || !app.innerHTML.includes('安全培训开始报名')) errors.push('Banner 推送列表字段或预置数据不正确');
  if (!app.innerHTML.includes('生效中') || !app.innerHTML.includes('未开始') || !app.innerHTML.includes('已停用')) errors.push('Banner 推送未覆盖生效中/未开始/已停用状态');
  if (!app.innerHTML.includes('页面说明') || !app.innerHTML.includes('推送类型') || !app.innerHTML.includes('生效时段') || !app.innerHTML.includes('用户端仅展示生效中')) errors.push('右侧页面说明未同步 Banner 推送规则');
  visit('#/form/banners/new');
  if (!app.innerHTML.includes('推送类型') || !app.innerHTML.includes('关联内容') || !app.innerHTML.includes('datetime-local') || !app.innerHTML.includes('ACT-01')) errors.push('Banner 推送新建表单缺少类型、关联内容或生效时间');
  if (!app.innerHTML.includes('2026 年鄞州区无人机飞行安全培训') || !app.innerHTML.includes('面向个人飞手、企业经办人')) errors.push('选择活动内容后未自动代入标题和简介');
  changeDraft('type', '低空安全普法');
  if (!app.innerHTML.includes('LAW-01') || !app.innerHTML.includes('无人驾驶航空器飞行安全提示')) errors.push('切换推送类型后未带入普法列表内容');
  visit('#/form/banners/BAN-01');
  if (!app.innerHTML.includes('安全培训开始报名') || !app.innerHTML.includes('ACT-01')) errors.push('Banner 推送编辑页未回显已保存文案');
  visit('#/guides');
  ...
  visit('#/volunteers');
  if (!app.innerHTML.includes('form/volunteers/new') || !app.innerHTML.includes('在册状态') || !app.innerHTML.includes('常住地址') || app.innerHTML.includes('所属区域') || app.innerHTML.includes('RID 配发状态') || /来源/.test(app.innerHTML.match(/data-table[\s\S]*?<\/table>/)?.[0] || '')) errors.push('后台志愿者管理缺少新建入口、常住地址列，或仍展示所属区域/RID 配发状态/来源列');
  if (app.innerHTML.includes('id="street-filter"') || (app.innerHTML.includes('id="area-filter"') && app.innerHTML.includes('filter-bar'))) errors.push('志愿者列表仍使用独立的区/街道下拉筛选，未合并为常住地址级联');
  click({ action: 'toggle-volunteer-address-filter' });
  if (!app.innerHTML.includes('data-action="pick-volunteer-address-filter"') || !app.innerHTML.includes('data-level="district"') || app.innerHTML.includes('data-level="province"')) errors.push('志愿者常住地址筛选未展开市/区/街道级联面板');
  click({ action: 'pick-volunteer-address-filter', level: 'district', value: '鄞州区' });
  if (!app.innerHTML.includes('data-level="street"')) errors.push('志愿者常住地址筛选选中区后未展开街道列');
  click({ action: 'pick-volunteer-address-filter', level: 'street', value: '下应街道' });
  if (!app.innerHTML.includes('宁波市 / 鄞州区 / 下应街道')) errors.push('志愿者常住地址筛选选中后未回填');
  click({ action: 'reset-filter' });
  if (!app.innerHTML.includes('admin-cascader-trigger is-placeholder') || !/admin-cascader-trigger is-placeholder[^>]*><span>全部<\/span>/.test(app.innerHTML)) errors.push('志愿者常住地址筛选重置后未恢复为全部');
  visit('#/form/volunteers/new');
  if (!app.innerHTML.includes('从用户选择') || !app.innerHTML.includes('手工录入') || !app.innerHTML.includes('选择用户') || !app.innerHTML.includes('data-action="volunteer-entry-mode"') || !app.innerHTML.includes('姓名 / 手机号') || !app.innerHTML.includes('device-picker') || !app.innerHTML.includes('常住地址') || !app.innerHTML.includes('admin-cascader') || !app.innerHTML.includes('data-action="toggle-address-cascader"') || !app.innerHTML.includes('data-action="pick-address-cascader"') || !app.innerHTML.includes('data-draft-field="addressDetail"') || !app.innerHTML.includes('data-draft-field="street"') || app.innerHTML.includes('admin-region-panel') || app.innerHTML.includes('region-cascade-select') || app.innerHTML.includes('data-action="toggle-region-cascader"')) errors.push('添加志愿者缺少从用户选择/手工录入双模式、用户搜索或常住地址级联选择器');
  click({ action: 'toggle-address-cascader' });
  if (!app.innerHTML.includes('admin-cascader-panel') || !app.innerHTML.includes('data-level="city"') || !app.innerHTML.includes('data-level="district"') || app.innerHTML.includes('data-level="province"') || app.innerHTML.includes('admin-region-panel')) errors.push('常住地址级联选择器未展开市/区/街道面板');
  click({ action: 'pick-address-cascader', level: 'district', value: '鄞州区' });
  if (!/admin-cascader-panel[^"]*\bhas-district\b/u.test(app.innerHTML) || !app.innerHTML.includes('data-level="street"')) errors.push('常住地址级联选择器选中区后未展开街道列');
  click({ action: 'pick-address-cascader', level: 'street', value: '下应街道' });
  if (!app.innerHTML.includes('宁波市 / 鄞州区 / 下应街道') || app.innerHTML.includes('admin-cascader is-open')) errors.push('常住地址级联选择器选中街道后未回填');
  click({ action: 'volunteer-pick-user', id: 'USR-001' });
  if (!app.innerHTML.includes('已选用户') || !app.innerHTML.includes('下应街道') || !app.innerHTML.includes('示范路 128 号') || !app.innerHTML.includes('宁波市 / 鄞州区 / 下应街道')) errors.push('从个人用户选择志愿者后未带入常住地址');
  ...
  visit('#/addresses');
  if (!app.innerHTML.includes('地址配置') || !app.innerHTML.includes('新增区') || !app.innerHTML.includes('海曙区') || !app.innerHTML.includes('鄞州区') || !app.innerHTML.includes('街道配置') || !app.innerHTML.includes('data-modal="address-streets"') || !['编号', '名称', '排序', '状态', '更新时间'].every((field) => app.innerHTML.includes(field))) errors.push('地址配置区列表字段或街道配置入口不正确');
  {
    const flightNav = app.innerHTML.match(/data-group="飞行计划管理"[\s\S]*?<\/div><\/div>/u)?.[0] || '';
    if (!['data-go="flights"', 'data-go="addresses"', 'data-go="flight-activity-types"'].every((item) => flightNav.includes(item))) errors.push('地址配置/飞行活动类型配置未归入侧栏「飞行计划管理」');
    if (['data-go="streets"', 'data-go="districts"'].some((item) => flightNav.includes(item))) errors.push('街道配置/市区配置仍作为独立侧栏入口存在');
    const droneNav = app.innerHTML.match(/data-group="无人机后台管理"[\s\S]*?<\/div><\/div>/u)?.[0] || '';
    if (['data-go="addresses"', 'data-go="streets"', 'data-go="districts"', 'data-go="flight-activity-types"'].some((item) => droneNav.includes(item))) errors.push('地址/飞行活动类型配置仍留在「无人机后台管理」');
  }
  click({ action: 'modal', modal: 'edit-config', key: 'addresses-district', item: 'new' });
  if (!app.innerHTML.includes('新增区') || !app.innerHTML.includes('data-draft-field="name"') || !app.innerHTML.includes('submit-config')) errors.push('地址配置新增区弹窗不正确');
  click({ action: 'close-modal' });
  const yinzhouDistrictId = (global.LowAltitudeMock?.addressConfigs || []).find((item) => item.name === '鄞州区')?.id || 'DST-05';
  click({ action: 'modal', modal: 'address-streets', district: yinzhouDistrictId });
  if (!app.innerHTML.includes('街道配置 · 鄞州区') || !app.innerHTML.includes('钟公庙街道') || !['编号', '街道名称', '排序', '状态', '更新时间'].every((field) => app.innerHTML.includes(field))) errors.push('地址配置街道配置弹窗字段或预置数据不正确');
  click({ action: 'modal', modal: 'edit-address-street', district: yinzhouDistrictId, item: 'new' });
  if (!app.innerHTML.includes('新增街道') || !app.innerHTML.includes('submit-address-street') || !app.innerHTML.includes('鄞州区')) errors.push('地址配置新增街道弹窗不正确');
  click({ action: 'close-modal' });
  visit('#/streets');
  if (!app.innerHTML.includes('地址配置') || !app.innerHTML.includes('新增区') || app.innerHTML.includes('>市区配置<')) errors.push('旧路由 #/streets 未重定向到地址配置');
  visit('#/districts');
  if (!app.innerHTML.includes('地址配置') || !app.innerHTML.includes('新增区') || app.innerHTML.includes('>市区配置<')) errors.push('旧路由 #/districts 未重定向到地址配置');
  visit('#/flight-activity-types');
  if (!app.innerHTML.includes('飞行活动类型配置') || !app.innerHTML.includes('新增类型') || !app.innerHTML.includes('一般飞行活动') || !app.innerHTML.includes('特殊飞行活动')) errors.push('飞行活动类型配置列表或预置数据不正确');
  click({ action: 'modal', modal: 'edit-config', key: 'flight-activity-types', item: 'new' });
  if (!app.innerHTML.includes('新增飞行活动类型') || !app.innerHTML.includes('submit-config')) errors.push('飞行活动类型配置新增弹窗不正确');
  click({ action: 'close-modal' });
  visit('#/license-types');
  if (!app.innerHTML.includes('执照分类管理') || !app.innerHTML.includes('新增分类') || !app.innerHTML.includes('民用无人驾驶航空器操控员执照') || !app.innerHTML.includes('查看数据') || !app.innerHTML.includes('分类数据')) errors.push('执照分类管理列表、预置分类或分类数据入口缺失');
  {
    const userNav = app.innerHTML.match(/data-group="用户和企业管理"[\s\S]*?<\/div><\/div>/u)?.[0] || '';
    if (!userNav.includes('data-go="license-types"')) errors.push('执照分类管理未归入侧栏「用户和企业管理」');
  }
  click({ action: 'modal', modal: 'edit-config', key: 'license-types', item: 'new' });
  if (!app.innerHTML.includes('新增执照分类') || !app.innerHTML.includes('submit-config')) errors.push('执照分类新增弹窗不正确');
  click({ action: 'close-modal' });
  click({ action: 'license-tab', value: 'records' });
  if (!app.innerHTML.includes('李女士') || !app.innerHTML.includes('操控员执照.png') || !app.innerHTML.includes('data-go="detail/licenses/LIC-01"')) errors.push('执照分类管理未展示分类下已上传数据');
  visit('#/detail/license-types/LIC-T-01');
  if (!app.innerHTML.includes('执照分类详情') || !app.innerHTML.includes('分类下数据') || !app.innerHTML.includes('LIC-01')) errors.push('执照分类详情未展示分类下数据');
  visit('#/detail/licenses/LIC-01');
  if (!app.innerHTML.includes('执照数据详情') || !app.innerHTML.includes('民用无人驾驶航空器操控员执照') || app.innerHTML.includes('data-go="form/licenses/')) errors.push('执照数据详情缺失或仍提供编辑入口');
  const routes = {users:'用户管理',companies:'企业管理',blacklist:'黑名单','license-types':'执照分类管理','sys-users':'用户管理',roles:'角色管理',menus:'菜单管理',dicts:'字典管理',certificates:'UOM 登记证','drone-blacklist':'无人机黑名单',verification:'设备核查',addresses:'地址配置','flight-activity-types':'飞行活动类型配置',flights:'飞行计划',activities:'活动管理',laws:'低空安全普法',news:'新闻公告',banners:'Banner 推送',guides:'操作手册',faq:'常见问题',volunteers:'志愿者名册',messages:'消息管理',feedback:'意见反馈'};
  Object.entries(routes).forEach(([name, heading]) => {
    visit(`#/${name}`);
