...
runtime('apps/admin/app.js', '#/login', ({ app, visit, click, change, search, inputDraft, changeDraft, inputLogin }) => {
  ...
  if (app.innerHTML.includes('data-group="肩灯配发管理"') || app.innerHTML.includes('data-group="侦测预警"') || /nav-link[^>]*>[\s\S]*?RID模块/u.test(app.innerHTML) || /nav-link[^>]*>[\s\S]*?外部接口/u.test(app.innerHTML)) errors.push('后台侧栏仍保留已移除的 RID模块 / 肩灯配发 / 侦测预警 / 外部接口');
  visit('#/statistics');
  ...
  const removedRoutes = { situation: '低空态势', 'detection-devices': '侦测设备', watchlists: '黑白名单', fences: '电子围栏', disposal: '处置日志', 'detect-stats': '侦测统计', 'rid-modules': 'RID模块', 'shoulder-lights': '肩灯配发', alerts: '侦测预警', interface: '外部接口' };
  Object.entries(removedRoutes).forEach(([name, title]) => {
    visit(`#/${name}`);
    if (!app.innerHTML.includes('鄞州低空治理工作台')) errors.push(`已移除路由未回退工作台：${name}`);
    if (app.innerHTML.includes(`>${title}<`) || app.innerHTML.includes('鄞州区低空运行态势') || app.innerHTML.includes('肩灯感知自动比对闭环') || app.innerHTML.includes('侦测数据统计') || app.innerHTML.includes('RID-YZ-001') || app.innerHTML.includes('配发台账') || app.innerHTML.includes('市级低空飞行服务管理平台')) errors.push(`后台已移除的路由仍渲染业务页：${name}`);
  });
  ...
  if (!app.innerHTML.includes('从用户选择') || !app.innerHTML.includes('手工录入') || !app.innerHTML.includes('选择用户') || !app.innerHTML.includes('data-action="volunteer-entry-mode"') || !app.innerHTML.includes('姓名 / 手机号') || !app.innerHTML.includes('device-picker')) errors.push('添加志愿者缺少从用户选择/手工录入双模式或用户搜索');
  click({ action: 'volunteer-entry-mode', value: 'manual' });
  if (!app.innerHTML.includes('placeholder="如 138****0000"') || app.innerHTML.includes('device-picker-list')) errors.push('切换手工录入后姓名手机号应为可编辑');
  click({ action: 'volunteer-entry-mode', value: 'user' });
  if (!app.innerHTML.includes('选择用户') || !app.innerHTML.includes('data-action="volunteer-pick-user"') || !app.innerHTML.includes('姓名 / 手机号')) errors.push('切回从用户选择后应展示可搜索用户列表');
  click({ action: 'volunteer-pick-user', id: 'USR-001' });
  ...
  if (!app.innerHTML.includes('完成核查')) errors.push('待核查记录应提供完成核查入口');
  click({ action: 'request-change', key: 'verification', id: 'CHK-002' });
  if (!app.innerHTML.includes('完成核查') || !app.innerHTML.includes('处理意见') || !app.innerHTML.includes('submit-complete-verification') || !app.innerHTML.includes('<option>通过</option>') || !app.innerHTML.includes('<option>不通过</option>')) errors.push('完成核查弹窗应选择通过/不通过并填写处理意见');
  click({ action: 'close-modal' });
  visit('#/form/verification/new');
  if (!['从台账选择', '临时新增', '设备序列号', '核查类型', '核查方式', '核查地点', '核查结果', '问题描述', '处理意见', '计划跟进日期', '保存核查', 'device-picker'].every((field) => app.innerHTML.includes(field))) errors.push('设备核查新增表单字段或设备选择交互不正确');
  if (!app.innerHTML.includes('<option selected>待核查</option>') || app.innerHTML.includes('required data-draft-field="suggestion"') || app.innerHTML.includes('待补充')) errors.push('新增核查默认待核查时处理意见不应必填');
  if (app.innerHTML.includes('<select required data-draft-field="droneId">') || app.innerHTML.includes('核查对象/设备') || (app.innerHTML.includes('核查编号') && app.innerHTML.includes('data-draft-field="id"'))) errors.push('设备核查新增表单仍使用旧下拉或与列表字段完全统一');
  if (!app.innerHTML.includes('device-picker-pager') || !app.innerHTML.includes('每页 5 条') || !app.innerHTML.includes('data-action="picker-page"')) errors.push('设备选择列表缺少每页 5 条分页');
  if ((app.innerHTML.match(/data-action="verification-pick-drone"/g) || []).length > 5) errors.push('设备选择列表未按每页 5 条截断');
  click({ action: 'picker-page', page: '2' });
  if (!app.innerHTML.includes('第 2/') || !app.innerHTML.includes('data-action="verification-pick-drone"')) errors.push('设备选择翻页失败');
  click({ action: 'verification-device-mode', value: 'manual' });
  if (!app.innerHTML.includes('临时设备') || !app.innerHTML.includes('placeholder="如：云翼 M30"') || !app.innerHTML.includes('placeholder="如：SN-****-0192"')) errors.push('切换临时新增后应展示无人机名称与设备序列号录入');
  click({ action: 'verification-device-mode', value: 'ledger' });
  if (!app.innerHTML.includes('device-picker-list') || !app.innerHTML.includes('data-action="verification-pick-drone"') || !app.innerHTML.includes('每页 5 条')) errors.push('切回从台账选择后应展示可搜索分页设备列表');
  click({ action: 'verification-pick-drone', id: 'DR-001' });
  ...
  const routes = {users:'用户管理',companies:'企业管理',blacklist:'黑名单','sys-users':'用户管理',roles:'角色管理',menus:'菜单管理',dicts:'字典管理',certificates:'UOM 登记证',verification:'设备核查',flights:'飞行计划',activities:'活动管理',laws:'低空安全普法',news:'新闻公告',guides:'操作手册',faq:'常见问题',volunteers:'志愿者名册',messages:'消息管理',feedback:'意见反馈'};
  Object.entries(routes).forEach(([name, heading]) => {
    visit(`#/${name}`);
